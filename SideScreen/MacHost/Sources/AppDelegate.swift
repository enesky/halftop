...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private lazy var launchOptions = LaunchOptions(arguments: CommandLine.arguments)
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        guard urls.contains(where: LaunchOptions.requestsAutoStartUSB(url:)) else { return }
        Task { [weak self] in
            await self?.autoStartUSB()
        }
    }

    func autoStartUSB() async {
        await MainActor.run {
            self.settings.connectionMode = .usb
        }
        if settings.isRunning {
            debugLog("Auto-start USB requested while already running")
            return
        }
        await startServer()
    }
